...
const runSolidityTests = async ({ testFiles, chainType, grep, noCompile, verbosity }, hre) => {
    ...
    const { testRootPaths } = await hre.tasks
        .getTask("compile")
        .run({
        quiet: true,
        force: false,
    ...
    edrArtifacts.forEach(({ userSourceName, edrAtifact }) => {
        if (testRootPaths.includes(resolveFromRoot(hre.config.paths.root, userSourceName)) &&
    ...
    const testSuiteIds = edrArtifacts
        .filter(({ userSourceName }) => testRootPaths.includes(resolveFromRoot(hre.config.paths.root, userSourceName)))
